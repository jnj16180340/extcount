...
fn main() {
    ...
    for entry in entries {
        if let Ok(entry) = entry {
            process_entry(&entry.path(), &mut extension_counts, &mut extension_to_mime, &mut extension_to_stats);
        }
    }
    ...
    for (ext, count) in &extension_counts {
        ...
        let stats_str = match extension_to_stats.get(ext) {
            Some(stats_set) => format!("{:.2} ± {:.2}", (stats_set.mean().unwrap())/1024.0, (stats_set.sample_standard_deviation().unwrap_or(f64::NAN))/1024.0),
            None => format!("{:?}", ""),
        };
...
fn process_entry(path: &Path, counts: &mut HashMap<String, u32>, mime_map: &mut HashMap<String, HashSet<String>>, stats_map: &mut HashMap<String, Stats>) {
    ...
    if path.is_dir() {
        ...
        *counts.entry(ext_str.clone()).or_insert(0) += 1;
        ...
        mime_map.entry(ext_str.clone()).or_insert_with(HashSet::new).insert(mime);

        let file_size = path.metadata().expect("file metadata call failed").size();
        stats_map.entry(ext_str).or_insert_with(Stats::new).update(file_size as f64).unwrap();
